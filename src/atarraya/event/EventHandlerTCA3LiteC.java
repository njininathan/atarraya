...
public class EventHandlerTCA3LiteC implements EventHandlerSW,constants{
    ...
    public EventHandlerTCA3LiteC(atarraya_frame _father) {
    ...
    public void HandleEvent(event_sim e){
        ...
        switch(code){
            ...
            case PARENT_RECOGNITION:
                ...
                if(destination == receiver && !getNode(receiver).isSleeping(temp_tree)){
                        ...
                        if(getNode(receiver).getState(temp_tree) != S_ACTIVE){
                                ...
                                broadcast(temp_clock+getRandom(MAX_TX_DELAY_RANDOM), temp_clock, receiver, -1, SENSING_COVERED_MESSAGE, temp_data,temp_tree);
            ...
            case HELLO:
                ...
                if(getNode(receiver).getState(temp_tree) != S_ACTIVE && getNode(receiver).getState(temp_tree) !=S_ACTIVE_CANDIDATE && !getNode(receiver).isSleeping(temp_tree)){
                        ...
                        //If the sender's parent is the receivers's parent too... if the nodes are brothers...
                            if(temp_data_int3 == getNode(receiver).getDefaultGateway(temp_tree)){
                                if(getNode(receiver).getState(temp_tree) == S_WAITING_ACTIVE){
                                    ...
                                    getNode(receiver).setState(S_SLEEP_PRECANDIDATE,temp_tree);
            ...
            case PARENT_RECOG_TIME_OUT:
                ...
                if(getNode(sender).getState(temp_tree) != S_ACTIVE)
                {
                    if(getNode(sender).isSensing_covered(temp_tree)){
                        ...
                        getNode(sender).setEnergyState(STATE_SLEEP);
                    }else{
                        getNode(sender).setState(S_SLEEP_CANDIDATE, temp_tree);
                        metric = getNode(sender).getMetric(temp_tree);
                        pushEvent(new event_sim(temp_clock+TIMEOUT_GO_DEAD+((1-metric)*30.0)+getRandom(PROCESSING_DELAY), temp_clock, sender, sender, DELAYED_RADIO_OFF, "",temp_tree,type));
                    }
                 }
            ...
            case DELAYED_RADIO_OFF_2:
                ...
                if(temp_data_int  == temp_data_int2)
                    ...
                    pushEvent(new event_sim(temp_clock+RADIO_OFF_DELAY_PER_TREE+getRandom(PROCESSING_DELAY), temp_clock, sender, sender, DELAYED_RADIO_OFF_2, "",temp_tree,type));
            ...
            case DELAYED_RADIO_OFF:
                ...
                if(getNode(sender).getState(temp_tree) == S_SLEEP_CANDIDATE)
                    if(getNode(sender).isSensing_covered(temp_tree)){
                        ...
                        //getNode(sender).setRadioOn(temp_tree, true);
                        getNode(sender).setEnergyState(STATE_READY);
                        broadcast(temp_clock+getRandom(MAX_TX_DELAY_RANDOM), temp_clock, sender, -1, SENSING_COVERED_MESSAGE, temp_data,temp_tree);
                        
                        //Initiate the TM protocol only on the active nodes of the topology, except on the local
                        if(TM_Selected && (father.getTMType() != TM_ENERGY || getNode(sender).isSink(temp_tree))){
                            //pushEvent(new event_sim(temp_clock+DELTA_TIME, temp_clock, receiver, receiver, INIT_EVENT, "",temp_tree,TM_PROTOCOL));
                            pushEvent(new event_sim(temp_clock+DELTA_TIME, temp_clock, sender, sender, INIT_EVENT, "",getNode(sender).getActiveTree(),TM_PROTOCOL));
                        }

                        //Initiate the sensor querying protocol only on the active nodes of the topology!!
                        if(SD_Selected  && getNode(sender).getActiveTree() == temp_tree){
                            pushEvent(new event_sim(temp_clock+DELTA_TIME, temp_clock, sender, sender, INIT_EVENT, "",temp_tree,SENSOR_PROTOCOL));
                        }

                        //Initiate the COMM  protocol only on the active nodes of the topology!!
                        if(COMM_Selected){
                            pushEvent(new event_sim(temp_clock+DELTA_TIME, temp_clock, sender, sender, INIT_EVENT, "",temp_tree,COMM_PROTOCOL));
                        }
